...
// Series recomendadas (con enlaces de YouTube que funcionan)
...
export const SeriesSection: React.FC<SeriesSectionProps> = ({ onPlaySerie }) => {
  ...
  // Filtrar series por búsqueda y fuente
  ...
  // Manejar reproducción de URL personalizada
  const handleCustomUrl = () => {
    if (!customUrl.trim()) return;
    
    // Verificar si es YouTube
    if (customUrl.includes('youtube.com') || customUrl.includes('youtu.be')) {
      const result = extractYouTubeId(customUrl);
      if (result && onPlaySerie) {
        onPlaySerie(customUrl, 'URL personalizada', result.isPlaylist);
        setCustomUrl('');
      } else {
        alert('No se pudo extraer el ID del video. Verificá la URL.');
      }
    } else {
      // URL directa (Plex, Pluto, etc.)
      if (onPlaySerie) {
        onPlaySerie(customUrl, 'Contenido externo', false);
        setCustomUrl('');
      }
    }
  };
  ...
  return (
    <div style={{
          ...
          >
            <div style={{
              ...
              background: serie.source === 'youtube' 
                ...
                : 'linear-gradient(135deg, #ffaa33, #ff8844)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              fontSize: '48px'
            }}>
              {serie.source === 'youtube' ? '🎬' : serie.source === 'plex' ? '🎬' : '📡'}
            </div>
            <div style={{ padding: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', marginBottom: '8px', fontSize: '12px', color: '#888' }}>
                ...
                {serie.isPlaylist && <span>📀 Playlist</span>}
              </div>
              ...
              >
                {serie.source === 'youtube' ? '▶ Ver en YouTube' : '▶ Reproducir'}
              </button>
            </div>
          </div>
        ))}
      ...
      <div style={{
        ...
        💡 Podés pegar cualquier URL de YouTube, Plex o Pluto TV en el campo superior
      </div>
    </div>
  );
};